...
# Benchmark script for Sokoban solver using A* algorithm
# Results are saved to benchmark_results_astar.csv

LEVELS="01 02 03 04 05 06 07 08 09 10 11 12 13 14 15"
METHOD="astar"

# Create or overwrite the results file with headers
echo "Level,Method,Success,Steps,Time(s),NodesExplored" > benchmark_results_astar.csv

for level in $LEVELS; do
    echo "Running level $level with A*..."
    ...
    echo "$level,$METHOD,$success,$steps,$time,$nodes" >> benchmark_results_astar.csv
    
    # Take a short break between runs
    sleep 1
done

echo "A* Benchmark complete. Results saved to benchmark_results_astar.csv"
...
done < benchmark_results_astar.csv
